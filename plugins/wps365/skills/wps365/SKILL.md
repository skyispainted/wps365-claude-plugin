...
**在使用任何 wps365 功能前，必须先运行下面的初始化脚本。** 不能直接运行 `python -m wps_credential_manager` 或 `from wpsv7client import`，否则会因为 PYTHONPATH 未设置而报错。

运行以下脚本完成环境初始化（自动安装依赖 + 设置 PYTHONPATH）：

```bash
python3 -c "from cryptography.hazmat.primitives.ciphers.aead import AESGCM" 2>/dev/null || pip3 install cryptography 2>/dev/null || pip install cryptography; SCRIPTS_DIR=$(find ~/.claude/plugins/cache/wps365-marketplace/wps365 -maxdepth 4 -type d -name scripts 2>/dev/null | head -1); if [ -n "$SCRIPTS_DIR" ]; then export PYTHONPATH="$SCRIPTS_DIR${PYTHONPATH:+:$PYTHONPATH}"; fi; python3 -c "from wpsv7client import get_current_user; print('wps365 ready')"
```

**验证：** 看到 `wps365 ready` 表示初始化成功。之后才能使用下面的功能。

> 注意：Windows 系统上 `find` 命令不可用时，需要手动定位插件目录。插件安装在 `%USERPROFILE%\.claude\plugins\cache\wps365-marketplace\wps365\<version>\skills\wps365\scripts`，将该路径添加到 PYTHONPATH 环境变量即可。

## 认证（首次使用）

```bash
python3 -m wps_credential_manager login
```

在浏览器中完成 WPS 账号登录后，`wps_sid` 会自动存储并在过期时无感刷新。

## 凭证管理

```bash
python3 -m wps_credential_manager status   # 查看当前凭证状态
python3 -m wps_credential_manager refresh  # 手动刷新 token
python3 -m wps_credential_manager logout   # 清除凭证（重新登录）
python3 -m wps_credential_manager test     # 测试凭证是否有效
```
...
- 如果刷新失败，提示用户重新运行 `python3 -m wps_credential_manager login`。
