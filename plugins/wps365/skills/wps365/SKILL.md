...
**每次使用技能前，代理须先执行此初始化步骤。** 如果模块导入成功，跳过此步。

```bash
# 1. 安装 Python 依赖
python3 -c "from cryptography.hazmat.primitives.ciphers.aead import AESGCM" 2>/dev/null || pip3 install cryptography 2>/dev/null || pip install cryptography

# 2. 找到插件脚本目录并设置 PYTHONPATH
# Claude Code 插件安装路径在 ~/.claude/plugins/cache/wps365-marketplace/wps365/*/skills/wps365/scripts
SCRIPTS_DIR=$(find ~/.claude/plugins/cache/wps365-marketplace/wps365 -maxdepth 4 -type d -name scripts 2>/dev/null | head -1)
if [ -n "$SCRIPTS_DIR" ]; then
    export PYTHONPATH="$SCRIPTS_DIR${PYTHONPATH:+:$PYTHONPATH}"
fi

# 3. 验证
python3 -c "from wpsv7client import get_current_user; print('wps365 ready')"
```

初始化后，所有 `python3 -m` 和 `from wpsv7client import` 命令均可直接使用。
